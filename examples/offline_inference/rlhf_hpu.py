...
class MyWorker(HPUWorker):
    """
    The `MyWorker` class inherits from `HPUWorker` to provide custom functions.
...
train_model = AutoModelForCausalLM.from_pretrained("/models/Qwen2.5-Math-1.5B")
train_model.to("hpu")
"""
...
for name, p in train_model.named_parameters():
    ...
    model_update_group.broadcast(p, src=0)
    ray.get(handle)
