...
def sample_llmperf_requests(
    ...
    for i in range(num_prompts):
        ret = -1
        while ret <= 0:
            ret = int(random.gauss(input_len_mean, input_stddev))
        input_lens.append(ret)
    
    for i in range(num_prompts):
        ret = -1
        while ret <= 0:
            ret = int(random.gauss(output_len_mean, output_stddev))
        output_lens.append(ret)

    input_requests = []
    for i in range(num_prompts):
        prompt = (
            "Randomly stream lines from the following text "
            f"with {output_lens[i]} output tokens. "
            "Don't generate eos tokens:\n\n"
        )

        while input_lens[i] < get_token_length(prompt):
            while ret <= 0:
                ret = int(random.gauss(input_len_mean, input_stddev))
            input_lens[i] = ret

        remaining_prompt_tokens = input_lens[i] - get_token_length(prompt)
        with open(dataset_path, encoding='utf-8') as f:
...
def main(args: argparse.Namespace):
    ...
    elif args.dataset_name == "llmperf":
        input_requests = sample_llmperf_requests(
            dataset_path=args.dataset_path,
            input_len_mean=args.llmperf_input_mean,
            output_len_mean=args.llmperf_output_mean,
            num_prompts=args.num_prompts,
            input_stddev=args.llmperf_input_stddev,
            output_stddev=args.llmperf_output_stddev,
            tokenizer=tokenizer,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="sharegpt",
        choices=["sharegpt", "sonnet", "random", "hf", "llmperf"],
    ...
    llmperf_group = parser.add_argument_group("llmperf dataset options")
    llmperf_group.add_argument(
        "--llmperf-input-mean",
        type=int,
        default=1024,
        help=
        "Number of input tokens per request, used only for llmperf sampling.",
    )
    llmperf_group.add_argument(
        "--llmperf-output-mean",
        type=int,
        default=128,
        help=
        "Number of output tokens per request, used only for llmperf sampling.",
    )
    llmperf_group.add_argument(
        "--llmperf-input-stddev",
        type=float,
        default=1.0,
        help="Value of sampled input stddev, "
        "used only for llmperf sampling.",
    )

    llmperf_group.add_argument(
        "--llmperf-output-stddev",
        type=float,
        default=1.0,
        help="Value of sampled output stddev, "
        "used only for llmperf sampling.",
    )
