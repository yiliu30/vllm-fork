...
class INCARKWNA16MoEMethod(MoeWNA16Method):
    ...
    def _ark_moe(
        ...
        return self.ark.moe(
            ...
            weights if weights.is_contiguous() else weights.contiguous(),
            num_tokens_per_expert,
            scales=scales if scales.is_contiguous() else scales.contiguous(),
            zeros=None,
    ...
    @staticmethod
    def _local_to_global_experts(layer, num_local_experts: int) -> list[int]:
        if layer.expert_map is None:
            return list(range(num_local_experts))
    ...
    def _make_compact_inputs(
        ...
        local_to_global = self._local_to_global_experts(layer, num_local_experts)
    ...
    def apply(
        ...
        expected_routes = topk_ids.numel()
        actual_routes = int(num_tokens_per_expert.sum().item())
        if actual_routes != expected_routes:
            raise RuntimeError(
                f"ARK MoE route count mismatch: "
                f"actual={actual_routes}, expected={expected_routes}, "
                f"topk_ids_shape={tuple(topk_ids.shape)}, "
                f"expert_map_is_none={layer.expert_map is None}"
    )
        ...
        # activated_size = (
        #     compact_w13.shape[-1] // 2
        #     if activation.is_gated
        #     else compact_w13.shape[-1]
        # )
        ...
        # )
        activation = layer.activation
        if activation.value == "silu":
            gate, up = compact_w13.chunk(2, dim=-1)
            compact_activated = torch.nn.functional.silu(gate) * up
        else:
            activated_size = (
                compact_w13.shape[-1] // 2
                if activation.is_gated
