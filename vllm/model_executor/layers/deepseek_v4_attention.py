...
class DeepseekV4MultiHeadLatentAttentionWrapper(PluggableLayer):
    ...
    def __init__(
        ...
        # SM100: INT32 packed scales → (1, 1, 128).
        # SM90/SM120: FP32 block scales → (1, 128, 128).
...
class DeepseekV4MLAAttention(nn.Module, AttentionLayerBase):
    ...
    def _sink_aware_reference_attention(
        ...
        self._finish_reference_attention(max_score, denom, acc, output)
    ...
    def _forward_sm120_swa_decode_triton(
        self,
        q: torch.Tensor,
        ...
        """Triton kernel path for SWA-only decode attention on SM120."""
        ...
        offsets = (
            torch.arange(T, device=q.device, dtype=torch.int32) * max_swa_len
        ).unsqueeze(1)
        indices = (indices + offsets).unsqueeze(1)
    ...
    def _forward_sm120_compressed_decode_triton(
        self,
        q: torch.Tensor,
        ...
        """Triton kernel path for compressed + SWA decode on SM120."""
        from vllm.v1.attention.ops.triton_mla_utils import (
            lse_merge,
            merge_attention_with_sink,
        )
        ...
        comp_offsets = (
            torch.arange(T, device=q.device, dtype=torch.int32)
            * compressed_topk
        ).unsqueeze(1)
    ...
    def _forward_sm120_prefill_triton(
        self,
        q: torch.Tensor,
        kv: torch.Tensor,
        ...
        """Triton kernel path for prefill attention on SM120."""
        from vllm.v1.attention.ops.triton_mla_utils import (
            merge_attention_with_sink,
        )
        from vllm.v1.attention.ops.triton_sparse_mla_kernel import (
    ...
    def _forward_sm120_compressed_decode_reference(
        ...
        _, swa_max_score, swa_denom, swa_acc = (
            self._new_reference_attention_state(q)
        )
        swa_lens = swa_metadata.decode_swa_lens[:num_decode_tokens]
        dequantize_and_gather_k_cache(
    ...
    def _forward_sm120_prefill_reference(
        ...
        for token_start in range(0, q.shape[0], query_chunk_size):
            ...
            self._finish_reference_attention(
                max_score,
                denom,
                acc,
    ...
    def _forward_decode(
        ...
        if _is_sm120_reference_attention_enabled():
            _use_triton = envs.VLLM_SM120_TRITON_MLA
            if swa_only:
                if _use_triton:
                    self._forward_sm120_swa_decode_triton(
                        q=q,
            ...
            if self.compress_ratio in (4, 128):
                ...
                if _use_triton:
                    self._forward_sm120_compressed_decode_triton(
                        q=q,
    ...
    def _forward_prefill(
        ...
        for chunk_idx in range(num_chunks):
            ...
            if _is_sm120_reference_attention_enabled():
                if envs.VLLM_SM120_TRITON_MLA:
                    self._forward_sm120_prefill_triton(
                        q=q[query_start:query_end],
                        kv=kv[:chunk_size],
